...
webpackEmptyAsyncContext.id = 659;
...
async function initWasm() {
    ...
    wasmReady = (async () => {
        try {
            ...
            const mod = await __webpack_require__(659)(wasmPath);
    ...
    return function nNDescent(data, leafArray, nNeighbors, nIters = 10, maxCandidates = 50, delta = 0.001, rho = 0.5, rpTreeInit = true) {
        if (useWasm && isWasmAvailable()) {
            try {
                ...
                console.warn('WASM NN-Descent failed, falling back to JavaScript implementation:', error);
            }
        }
